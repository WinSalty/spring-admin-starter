...
public enum ErrorCode {
    ...
    CREDENTIAL_BATCH_NOT_FOUND(4076, "凭证批次不存在"),
    CREDENTIAL_BATCH_STATUS_INVALID(4077, "凭证批次状态不允许当前操作"),
    CREDENTIAL_SECRET_INVALID(4078, "凭证无效"),
    CREDENTIAL_UNAVAILABLE(4079, "凭证不可使用"),
    CREDENTIAL_SECRET_MISSING(4080, "凭证安全密钥未配置"),
    CREDENTIAL_BATCH_SIZE_EXCEEDED(4081, "凭证批次数量超过上限"),
    CREDENTIAL_ITEM_NOT_FOUND(4082, "凭证明细不存在"),
    CREDENTIAL_REDEEM_LOCKED(4083, "凭证兑换失败次数过多，请稍后再试"),
    BENEFIT_TYPE_UNSUPPORTED(4084, "权益类型不支持"),
    ...
    CREDENTIAL_ITEM_STATUS_INVALID(4094, "凭证明细状态不允许当前操作"),
    CREDENTIAL_SECRET_DECRYPT_FAILED(4095, "凭证明文解密失败"),
    CREDENTIAL_EXTRACT_LINK_NOT_FOUND(4096, "凭证提取链接不存在"),
    CREDENTIAL_EXTRACT_LINK_EXPIRED(4097, "凭证提取链接已过期"),
    CREDENTIAL_EXTRACT_LINK_EXHAUSTED(4098, "凭证提取链接访问次数已用完"),
    CREDENTIAL_EXTRACT_LINK_DISABLED(4099, "凭证提取链接已停用"),
    CREDENTIAL_EXTRACT_LINK_LIMIT_INVALID(4100, "凭证提取链接访问次数不合法"),
    CREDENTIAL_EXTRACT_SECRET_MISSING(4101, "凭证提取链接安全密钥未配置");
