...
public interface SystemMapper {
    ...
    String USERS_SELECT = "SELECT u.id, u.record_code AS recordCode, 'users' AS moduleKey, IFNULL(u.nickname, u.username) AS name, u.username AS code, u.status, u.owner, u.description, u.avatar_url AS avatarUrl, "
            ...
            + "NULL AS dataScope, NULL AS userCount, NULL AS dictType, NULL AS itemCount, NULL AS cacheKey, NULL AS logType, NULL AS target, NULL AS ipAddress, NULL AS deviceInfo, NULL AS userAgent, NULL AS browser, NULL AS browserVersion, NULL AS osName, NULL AS osVersion, NULL AS deviceType, NULL AS deviceBrand, NULL AS requestInfo, NULL AS responseInfo, NULL AS result, NULL AS durationMs, "
    ...
    String LOGS_SELECT = "SELECT l.id, l.record_code AS recordCode, 'logs' AS moduleKey, l.name, l.code, l.status, l.owner, l.description, NULL AS avatarUrl, "
            + "NULL AS department, NULL AS departmentId, NULL AS roleNames, NULL AS roleCodes, NULL AS lastLoginAt, NULL AS dataScope, NULL AS userCount, NULL AS dictType, NULL AS itemCount, NULL AS cacheKey, l.log_type AS logType, l.target AS target, l.ip_address AS ipAddress, l.device_info AS deviceInfo, l.user_agent AS userAgent, l.browser AS browser, l.browser_version AS browserVersion, l.os_name AS osName, l.os_version AS osVersion, l.device_type AS deviceType, l.device_brand AS deviceBrand, l.request_info AS requestInfo, l.response_info AS responseInfo, l.result AS result, l.duration_ms AS durationMs, "
