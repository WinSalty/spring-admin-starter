...
public class RegisterMailServiceImpl implements RegisterMailService {
    ...
    private StandardMailTemplate buildTemplate(String verificationUrl, long ttlSeconds) {
        ...
        template.setSummary("你正在创建后台账号，请点击下方按钮完成邮箱验证，验证通过后即可回到注册页继续提交。");
        template.setDescription("本链接将在 " + ttlMinutes + " 分钟内有效，且仅能用于本次注册邮箱验证。");
        template.setActionText("验证邮箱并继续注册");
